...
matrices_MBV_lasso_parallel = function (data, N, pi, numCores){
  ...
  mat = foreach(1:N, .combine = rbind, .multicombine = TRUE, .packages = c("glmnet", "tidyverse")) %dopar%{
    s = split(data, pi)
    data_train = s$train
    data_test = s$test
    ...
    return(list(vect_coef = vect_coef, betaM_hat = betaM_hat, index = M_hat))
  ...
  return(list(m_M = matrice_M_hat, m_beta = matriceBetaM_hat, var_names = colnames(data[, -dim(data)[2]]), time = time))
}
#-----------------------------------------------------------------------------------------
...
matrices_MBV_aic_parallel = function (data, N, pi,  numCores, direction){
  ...
  mat = foreach(1:N, .combine = rbind, .multicombine = TRUE, .packages = c("MASS, tidyverse")) %dopar%{
...
vote2 = function(matrices, N, emp_alpha){
  ...
  return(list(gg = gg, m = M2_vote, beta = beta2, ic = ic, names_vars = matrices$var_names))
}
...
#--------------------------------------Mise en place des fonction du Packages --------------------------------------------------------------------------
...
#' @param N Number of splittings.
#' @param p_split Probabilities associated with the splittings.
...
#' @import mlbench
#' @import tictoc
#' @return Un objet de classe lmps
#'
#'@examples
#'
#' library(mlbench)
...
#'
#'@export
lmps = function(formula, data, method , N, p_split = 0.5, cores = NULL, direction = "backward"){
  # method = c("Lasso", "BIC")
  # vote = c("model", "coef")
